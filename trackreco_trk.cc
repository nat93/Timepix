...
const Double_t timecut      = 10000.0;  // [ns]
const Double_t plane_position[]     = {0.0,305.0,305.0+600.0,305.0+600.0+270.0};    // [mm]
const Double_t plane_position_err[] = {10.0,10.0,10.0,10.0};                        // [mm]
...
void function_1(TString inFileName, TString outFileName);
void function_2(TString inFileName, TString outFileName);
void function_3(TString inFileName, TString outFileName);
void function_4(TString inFileName, TString outFileName);
void function_5(TString inFileName, TString outFileName);
...
int main(int argc, char *argv[])
{
    if(argc != 4)
    {
        ...
        cout<<"[3] - function number:"<<endl;
        cout<<"         1 --> function_1() - Delta time between clusters"<<endl;
        cout<<"         2 --> function_2() - Integrated image"<<endl;
        cout<<"         3 --> function_3() - Alignment"<<endl;
        cout<<"         4 --> function_4() - Track reconstruction"<<endl;
        cout<<"         5 --> function_5() - Detector efficiency"<<endl;
        cout<<endl;
    }
    else
    {
        ...
        {
        case 1:
            function_1(argv[1],argv[2]);
            break;
        case 2:
            function_2(argv[1],argv[2]);
            break;
        case 3:
            function_3(argv[1],argv[2]);
            break;
        case 4:
            function_4(argv[1],argv[2]);
            break;
        case 5:
            function_5(argv[1],argv[2]);
            break;
        default:
            cout<<endl<<endl<<"--> NOTHING to DO =) <--"<<endl<<endl;
            break;
        }
    }
    return 0;
...
void function_2(TString inFileName, TString outFileName)
{
    ...
    for(Int_t i = 0 ; i < N_MAX_CHIP; i++)
    {
        cout<<"offset_x_ChipID["<<i<<"] = "<<offset_x_ChipID[i]<<";"<<endl;
        cout<<"offset_y_ChipID["<<i<<"] = "<<offset_y_ChipID[i]<<";"<<endl;
        cout<<"offset_errx_ChipID["<<i<<"] = "<<offset_errx_ChipID[i]<<";"<<endl;
        cout<<"offset_erry_ChipID["<<i<<"] = "<<offset_erry_ChipID[i]<<";"<<endl;
    }
    cout<<endl;
    //------------------------------------------------------------------------------------------------//
...
void function_3(TString inFileName, TString outFileName)
{
    ...
    Double_t offset_x_ChipID[N_MAX_CHIP], offset_errx_ChipID[N_MAX_CHIP], offset_y_ChipID[N_MAX_CHIP], offset_erry_ChipID[N_MAX_CHIP];
    offset_x_ChipID[0]      = 162.734;
    offset_y_ChipID[0]      = 117.921;
    offset_errx_ChipID[0]   = 0.245998;
    offset_erry_ChipID[0]   = 0.273639;
    offset_x_ChipID[1]      = -256.913;
    offset_y_ChipID[1]      = 210.226;
    offset_errx_ChipID[1]   = 0.298941;
    offset_erry_ChipID[1]   = 0.334195;
    offset_x_ChipID[2]      = 94.6631;
    offset_y_ChipID[2]      = 106.854;
    offset_errx_ChipID[2]   = 0.247186;
    offset_erry_ChipID[2]   = 0.275789;
    offset_x_ChipID[3]      = -266.222;
    offset_y_ChipID[3]      = 209.074;
    offset_errx_ChipID[3]   = 0.31189;
    offset_erry_ChipID[3]   = 0.345495;
...
void function_4(TString inFileName, TString outFileName)
{
    ...
    //---------------------------------------------//
    // After 20 iteration
    offset_xres_Plane[0]    = -0.208866;
    offset_errxres_Plane[0] =  0.0019705;
    offset_yres_Plane[0]    = -0.14916;
    offset_erryres_Plane[0] =  0.00234141;

    offset_xres_Plane[1]    =  0.291211;
    offset_errxres_Plane[1] =  0.00202469;
    offset_yres_Plane[1]    =  0.22062;
    offset_erryres_Plane[1] =  0.002361;

    offset_xres_Plane[2]    = -0.0780656;
    offset_errxres_Plane[2] =  0.00236186;
    offset_yres_Plane[2]    = -0.00532924;
    offset_erryres_Plane[2] =  0.00236614;

    offset_xres_Plane[3]    = -0.0687679;
    offset_errxres_Plane[3] =  0.00178602;
    offset_yres_Plane[3]    = -0.0954583;
    offset_erryres_Plane[3] =  0.00191939;
    //---------------------------------------------//
    ...
    Double_t slope_arm1_x_err, slope_arm2_x_err, slope_arm1_y_err, slope_arm2_y_err;
...
void function_5(TString inFileName, TString outFileName)
{
    ...
    Int_t nTracks[N_MAX_CHIP] = {};
    TGraphErrors* grTracksX = new TGraphErrors();
    TGraphErrors* grTracksY = new TGraphErrors();
    grTracksX->SetName("grTracksX");
    grTracksY->SetName("grTracksY");
    Int_t grTrackPoint = 0;
    const Int_t nMaxClusters = 100;
    Double_t x_coord[N_MAX_CHIP][nMaxClusters], y_coord[N_MAX_CHIP][nMaxClusters], errx_coord[N_MAX_CHIP][nMaxClusters], erry_coord[N_MAX_CHIP][nMaxClusters];
    TF1* fitTrack = new TF1("fitTrack","pol1",plane_position[0],plane_position[N_MAX_CHIP-1]);
    Double_t fitXpos,fitYpos;

    //------------------------------------------------------------------------------//
    //------------------------------- HISTOGRAMS -----------------------------------//
    //------------------------------------------------------------------------------//
    TH1D* h132 = new TH1D("h132","Plane0 Detection Efficiency vs R",40000,0,40000);
    ...
    for(Int_t i = 0; i < nEntriesClock; i++)
    {
        ...
        for(Int_t clockID = 0; clockID < N_MAX_CLOCKS+1; clockID++)
        {
                    ...
                    for(Int_t chipID = 0; chipID < N_MAX_CHIP; chipID++)
                    {
                        ...
                        for(Int_t kkl = clockID-clock_jitter; kkl <= clockID+clock_jitter; kkl++)
                        {
                            ...
                            for(UInt_t kkll = 0; kkll < clock_info[chipID][kkl]; kkll++)
                            {
                                ...
                                // In X dir. //
                                //-----------//
                                x_coord[GetPlaneID(chipID)][chip_ready[chipID]] = (x_pos-offset_x_ChipID[chipID])*PIXEL_SIZE-offset_xres_Plane[GetPlaneID(chipID)];
                                errx_coord[GetPlaneID(chipID)][chip_ready[chipID]] = TMath::Sqrt(TMath::Power(x_pos_err*PIXEL_SIZE,2) +
                                                                                                 TMath::Power(offset_errx_ChipID[chipID]*PIXEL_SIZE,2) +
                                                                                                 TMath::Power(offset_errxres_Plane[GetPlaneID(chipID)],2));
                                ...
                                // In Y dir. //
                                //-----------//
                                y_coord[GetPlaneID(chipID)][chip_ready[chipID]] = (y_pos-offset_y_ChipID[chipID])*PIXEL_SIZE-offset_yres_Plane[GetPlaneID(chipID)];
                                erry_coord[GetPlaneID(chipID)][chip_ready[chipID]] = TMath::Sqrt(TMath::Power(y_pos_err*PIXEL_SIZE,2) +
                                                                                                 TMath::Power(offset_erry_ChipID[chipID]*PIXEL_SIZE,2) +
                                                                                                 TMath::Power(offset_erryres_Plane[GetPlaneID(chipID)],2));
            ...
            // Calculate efficiency for each detector
            for(Int_t chipID = 0;  chipID < N_MAX_CHIP; chipID++)
            {
                grTrackPoint = 0;
                Bool_t track_is_ready = true;

                // To check if 3 of 4 are fired with only one cluster for a given clock
                for(Int_t chipIDtmp = 0; chipIDtmp < N_MAX_CHIP; chipIDtmp++)
                {
                    if(chipIDtmp == chipID) {continue;}
                    if(chip_ready[chipIDtmp] != 1) {track_is_ready = false;}
                }

                if(track_is_ready)
                {
                    // To put points according to a plane's number
                    for(Int_t planeIDtmp = 0; planeIDtmp < N_MAX_CHIP; planeIDtmp++)
                    {
                        if(planeIDtmp == GetPlaneID(chipID)) {continue;}

                        grTracksX->SetPoint(grTrackPoint,plane_position[planeIDtmp],x_coord[planeIDtmp][0]);
                        grTracksY->SetPoint(grTrackPoint,plane_position[planeIDtmp],y_coord[planeIDtmp][0]);
                        grTracksX->SetPointError(grTrackPoint,plane_position_err[planeIDtmp],errx_coord[planeIDtmp][0]);
                        grTracksY->SetPointError(grTrackPoint,plane_position_err[planeIDtmp],erry_coord[planeIDtmp][0]);
                        grTrackPoint++;
                    }
                    ...
                    fitXpos = fitTrack->Eval(plane_position[chipID]);
                    ...
                    {
                        for(Int_t iCluster = 0; iCluster < chip_ready[chipID]; iCluster++)
                        {
                            Double_t Rm = TMath::Sqrt( TMath::Power(x_coord[GetPlaneID(chipID)][iCluster]-fitXpos,2) + TMath::Power(y_coord[GetPlaneID(chipID)][iCluster]-fitYpos,2));
    ...
    for(Int_t chipID = 0;  chipID < N_MAX_CHIP; chipID++)
    {
        ...
        {
        case 0:
            h132->Scale(1.0/nTracks[chipID]);
            cout<<nTracks[chipID]<<endl;
            break;
        case 1:
            h133->Scale(1.0/nTracks[chipID]);
            cout<<nTracks[chipID]<<endl;
            break;
        case 2:
            h134->Scale(1.0/nTracks[chipID]);
            cout<<nTracks[chipID]<<endl;
            break;
        case 3:
            h135->Scale(1.0/nTracks[chipID]);
            cout<<nTracks[chipID]<<endl;
            break;
        }
    }
