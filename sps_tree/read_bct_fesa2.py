...
len_array = 0			# new lenght of the array, computed to be multiple of 100 (see the code)
user = 'SPS.USER.LHCMD2'
...
# The following values (injection delay and offset) are both in ms
injection_delay = 1015 # in ms
offset = int(time.mktime(datetime.datetime.strptime(sys.argv[2], '%Y_%m_%d').timetuple()))*1000 + injection_delay # in ms
	...
	for i in range(0, len(files)):
		with open(files[i]) as csvfile:
			...
			for row in reader:
				...
				timestamp = int(row['cycleStamp']) # in ns
				if number_of_meas != 0 and (row['SPS_USER'] == user):
					len_array = int(number_of_meas/100) * 100
					list_timestamp = numpy.array(list(map(float, row['measStamp'].split('|'))))*10**(ts_exponent + 9) # in ns
					intensity = numpy.array(list(map(float, row['totalIntensity'].split('|'))))*10**intensity_exp
					intensity_mean = numpy.mean(numpy.reshape(numpy.resize(intensity, len_array), (-1, 100)), axis = 1)
					intensity_std = numpy.std(numpy.reshape(numpy.resize(intensity, len_array), (-1, 100)), axis = 1)
					time = numpy.reshape(numpy.resize(list_timestamp, len_array), (-1, 100)) # in ns
					for l in range(0, len_array/100):
						output_file.write(str(int(((time[l][-1] + timestamp)*1E-6 - offset)*1E-3))+' '+str(intensity_mean[l])+' '+str(intensity_std[l])+'\n') # time in s
				else:
					for l in range(0, int(number_of_meas/100)):
