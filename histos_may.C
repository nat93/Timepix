...
int histos_1()//to plot all alignment runs on the same plots
{
    ...
    for(Int_t j = 1; j <= 66; j++)
    {
        if(j == 28 || j == 34 || j == 39 || j == 44 || j == 48 || j == 51 || j == 54 || j == 58 || j == 62)
        {
            TString filename = "/home/anatochi/Medipix/ROOT_FILES/H8_Test_Beam_2018_05_16_pions_RUN_HISTO_0_";
            filename += j;
            filename += ".root";
...
int histos_2()//to plot all runs with a septum runs and with a align run
{
    for(Int_t j = 1; j <= 66; j++)
    {
        ...
        TString canvasname_1 = "./canvas/prof_run59_run28_run";
        canvasname_1 += j;
        TCanvas* c_1 = new TCanvas(canvasname_1.Data(),canvasname_1.Data(),1920,1080);
        c_1->cd();
        h_1->Draw();
...
{
    TFile *_file_1 = TFile::Open("/home/anatochi/Medipix/ROOT_FILES/H8_Test_Beam_2018_05_16_pions_RUN_HISTO_0_16.root");
    TFile *_file_2 = TFile::Open("/home/anatochi/Medipix/ROOT_FILES/H8_Test_Beam_2018_05_16_pions_RUN_HISTO_0_28.root");
    ...
    Int_t mean1Y = fit_funct->GetParameter(1);
    h_2y->Fit(fit_funct,"RQ+");
    Int_t mean2Y = fit_funct->GetParameter(1);
